...
**在 Codex Desktop 中运行 Pi、Claude Code 和 Grok**

我们认为 **Codex Desktop** 提供了目前最好的桌面开发交互体验。

但 **Codex** 并不是唯一优秀的 **Agent Harness**，也有人偏好 **Claude Code**、**Pi Agent** 和 **Grok CLI**。
...
Pi、Claude Code 与 Grok 在同一个 Codex Desktop 中作为独立会话运行。各 Harness 支持的流式输出、思考过程、工具状态和审批会实时渲染。
...
- **Harness 侧**：Pi 使用官方 RPC，Claude Code 使用 Agent SDK / CLI，Grok CLI 使用官方 [ACP](https://agentclientprotocol.com/) v1 stdio；各 Adapter 再将可靠能力投影到 Desktop
