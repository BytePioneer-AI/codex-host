...
3. 启动 `web --no-open --host 127.0.0.1 --port 0`，等待原生 bootstrap，完成认证，再建立 HTTP/WebSocket 通信。托管进程使用自己的临时端口。

用户自己启动的 DSH Web（例如由 launchd 等监督服务固定在默认端口上的实例）与托管实例互不影响：codexhost 不探测、不 attach、不停止它，也不会向其发送会话内容，因此无需先关闭它。
