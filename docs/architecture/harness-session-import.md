...
设置 → 会话导入可登记 **Claude Code、Pi、Hermes** 和 **DSH `0.1.2-rc.1` / `0.1.5-rc.1` / `0.1.5-rc.2`** 的原生 Session。导入只建立 Host Thread 与原生 Session 的映射，不复制 Transcript、不转换 Harness、不发送用户 Turn；打开后仍通过对应 Adapter 的 `open({ kind: "resume" })` 恢复历史并继续会话。
...
- DSH 仅允许本机、codexhost 托管的精确 `0.1.2-rc.1`、`0.1.5-rc.1` 和 `0.1.5-rc.2`；Legacy 协议已移除，其他版本使用现有错误入口提示支持范围并推荐 `dsh-v0.1.5-rc.2`。
...
旧 `codexhost/deepseek/modern-session/list` / `import` 作为兼容别名保留在 Host，复用同一个 DSH importer 和通知去重集合；旧 list 仍返回 `{ candidates }`，同样仅支持上述三个版本。这些 Host RPC 别名不属于已移除的 DSH Legacy 协议。新 Renderer 只使用公共 RPC。旧 Host 未实现公共入口时显示不可用，不改走未经验证的原生桥接。存储读取失败显示“无法读取本地会话”，不再误报“不支持导入”。
...
- 导入只登记映射；打开 Thread 后才读取原生历史，并继续相同 Native Session ID。`0.1.2-rc.1` 使用 V0 日志；`0.1.5-rc.1` 与 `0.1.5-rc.2` 使用 V3 日志及独立 Assistant 流，系统消息参与原生历史引用但不展示为用户回合。
...
- 用户自己启动的 DSH Web 与 codexhost 托管的 Web 互不影响：托管实例使用自己的临时端口与 bootstrap 认证；认证后，codexhost 只对这个托管 origin 执行 `GET /` Portal 校验，再使用其公开 Session API。codexhost 不探测、不 attach、不停止外部实例，也不会向其发送会话内容。
