...
DSH V0/V3/V4 的可见原生思考增量也会实时展示。临时思考被最终消息修订、移除或被放弃时，旧 Item 明确标记取消；新的最终 Item 只显示原生权威内容。重连按原生 Assistant baseline 去重，冷恢复仅从 DSH 持久化历史读取最终思考。带非空 `command` 参数的 `pwsh` Tool 使用可展开命令框显示完整命令和有界输出；缺少有效命令时仍按普通 Tool 显示。
