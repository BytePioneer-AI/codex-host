...
### 上游 v0.6.0 兼容同步（2026-09-08）

同步到上游 `e8f8ecd`，保留 Kiro 的插件清单、通用 Harness 路由、模型恢复、Thinking、审批和 Usage 接线。当前实现状态以本节及后续实施补记为准，原始调研的未实现清单保留为历史记录。

- Codex 多账号的 Host 隔离及账号选择回调与 Kiro 模型选择分别保留；修复原模型恢复测试仍读取旧回调参数位置的问题。
- Usage 合并上游所选 Codex 账号显示与 Kiro 的 credits、独立 Context 百分比；套餐额度遵循上游独立入口。Kiro 的会话已记录消耗不是账号剩余额度，不将其填入新增的可选 `inspectAccount()` 接口。
- 上游 `turn/steer` 由 Host 取消旧 Turn、等待终态及持久化后启动替代 Turn。Kiro 继续使用原生取消与普通 Prompt；适配器回归验证同一个 Native Session 在取消终态后接受下一轮，不声明新增原生 steering 能力。真实 CLI 取消后继续的验收仍单独保留。
- 合并 Antigravity 静态命令目录与 Kiro 目录；修正草稿 `/compact` 的旧浏览器断言，创建 Thread 前展示但禁用，不尝试执行。
- 验证范围为类型检查、插件及 Renderer 构建、定向单元测试和使用模拟客户端的浏览器回归。未启动 Desktop、提交模型 Prompt 或进行跨平台原生验收。
...
- 工具保留前序参数及类型，只结束一次；只有成功终态携带的 Diff 才确认文件修改，审批前预览按后续补记处理，孤立的初始化工具更新不归入当前 Turn。
...
- 历史展示区分原生 `Say`、`Reasoning` 和 `Summary`。Reasoning 的 `...` 占位不再成为普通回答或抢占最终回答；有正文的公开 Reasoning 保留独立类型。Item 身份使用原生记录 ID，回退删除的分支不再混入当前历史。
- 原生 Fork 只接受有效上下文中的消息。压缩前的 `turn_end` 即使仍在 JSONL 中，也可能已经不能分叉；覆盖完整历史前缀的摘要可作为该前缀末轮的新边界，不能用于冒充更早轮次的精确边界。分叉子会话仅保留摘要时，通过原生 `parentSessionId` 与相同摘要 ID 只读恢复展示前缀，不改写原生文件。父历史缺失或边界不匹配时明确报错。被压缩掉的较早位置不再发布失效 checkpoint；原生返回 `data.details` 中的边界错误不再降为笼统的 Fork 失败。
- `/kiro-context` 和 `/kiro-usage` 改为标题、分节和 Markdown 表格。未知结果使用独立代码块，保留字段脱敏，不再使用只保留末尾 8,000 字符的日志函数处理业务正文。账户用量和上下文分类统计仍保持原生单位及口径。
- 定向测试覆盖最终回答、压缩与回退边界、分叉历史展示及命令格式；真实本机复测通过普通 Fork、原先失败的压缩后 Fork、跨进程恢复和两个查询命令。未提交模型 Prompt、未重启 Desktop；修复后原生 Desktop 画面仍需复测。

### 用量、压缩进度、提问与编辑复测（2026-09-06）

- **输入/输出及缓存用量**：本机 KAS 的内部消息解析包含 `inputTokens`、`outputTokens`、`cacheReadInputTokens` 和 `cacheWriteInputTokens`，但 ACP 的 `turn_completion` / 历史 `usage_summary` 只聚合原生计费单位和 credits。现有探针记录与本次测试会话 JSONL 中没有上述 token 字段。进一步仅为隔离子进程配置本机 OTLP 接收端：Haiku 4.5 提问/取消测试收到 214 种指标，`auto` 单轮测试收到 146 种指标，均没有 input/output/cache token 数值；没有保存原始遥测或修改全局配置。这证明当前已测路径尚无可供面板使用的可靠数据，不证明所有模型或未来版本永久不支持。上下文分类的估算 token 不能当成账单输入输出或缓存命中。
- **手动压缩进度**：命令现在立即返回接受回执，避免 Host 等待命令结束才放行事件；在调用原生压缩前发布 Item 开始事件，成功、失败或关闭时结束同一 Item。失败不会发布成功终态，命令完成后刷新用量。没有虚构百分比，完成时仍沿用 Desktop 原生压缩标记。
- **提问与取消**：Kiro 的原生 `userInput` 在 Spec 模式提供，默认 Vibe 模式的普通工具目录没有它。已通过 Adapter 实测选择题答案往返与等待提问时取消，走 `_kiro/userInput` → 公共 Question → Codex 提问界面协议，不需要仿照 agy 再安装 Hook。模式命令成功后显示当前所选模式。人工测试：先执行 `/kiro-spec`，再发送“请只调用一次 userInput 工具，让我在 Red 和 Blue 中选择；等待工具回复后只说我选的颜色，不读写文件、不调用其他工具。”选择一次以测回答；再次发起后点击停止以测取消；结束后用 `/kiro-vibe` 切回默认模式。
...
- 目录加载采用先返回缓存、再按需后台核对：按规范化工作目录合并并发探测，成功目录仅保存在当前 Adapter 内存中；到上次成功后的 24 小时或下个 UTC 月初（取较早者），下一次查询仍立即返回目录并启动一次后台更新，不设置空闲轮询定时器。后台结果供后续目录读取使用；升级或切换账号后需要立即更新当前菜单时，使用连接诊断中的显式刷新，该操作等待原生结果并重新加载菜单。暂时断网保留已知目录，后台重试至少间隔 5 分钟；明确未安装或需重新认证时清除缓存。首次无缓存仍需等待，关闭 Adapter 时清空缓存。不内置套餐模型名单，缓存不是模型授权依据，实际会话配置仍须经 Kiro 原生确认。
- 月初边界依据 Kiro 官方 Billing FAQ（`https://kiro.dev/docs/billing/related-questions/`）：账期于每月 1 日 00:00 UTC 开始。官方个人计费文档（`https://kiro.dev/docs/billing/`）说明升级立即生效、降级次月生效；月初是账期/额度边界，不是模型目录只会在该时点变化的保证，因此保留每日后台核对与显式刷新。
...
- Spec 的 `analyze-requirements` 虽通过原生 `requestPermission` 发送，实际是需求选择题，现在投影为 Question，不再因缺少拒绝选项而取消 Turn。选项值保留原生 ID，重复标签仅添加显示序号；非法答案不清除待回答问题，取消或关闭返回原生 cancelled。
- 已有 Thread 的模型不在最新目录时，保留真实生效模型及最新可选目录，允许用户显式改选，不自动切换模型。无效模型、选择中和选择失败时保持阻止发送；原生确认有效模型后恢复。有效的权限配置不随模型缺失一起降为错误。
- 原生 pending Diff 在审批前作为进行中的文件预览显示，审批说明保留文件路径；终态携带的 Diff 更新同一 Item 并确认成功。拒绝、失败、取消、关闭或缺少终态 Diff 时清除未确认预览，不将其计入成功修改。
- 定向回归覆盖需求选择往返及取消、模型失效后的失败重试与发送恢复、文件预览生命周期和公共协议投影；隔离浏览器使用合成 Kiro 目录验证模型菜单可选、失败可重试及成功后恢复发送。未提交真实模型 Prompt，未重启 Desktop；浏览器夹具不替代真实 Desktop 的最终审批画面验收。

## 1. 结论

### DSML 显示泄漏修复（2026-09-06）

人工工具遍历测试对应的 `deepseek-3.2` 原生记录中，20 条 `Say` 正文含独立、未闭合的 `<｜DSML｜function_calls` 前导行，同时存在独立的工具调用和结果记录。Adapter 现在为实时文本与历史显示共用有状态的过滤器，支持标记跨 chunk 拆分，不创建仅含空白的回答；不改写原生历史、不解析或执行 DSML。只处理上述独立行，保留正文中的引用、围栏代码块、完整标签、用户输入和工具结果。离线三字符分片回放保留 20 个工具开始事件、4 轮历史、7 条可见回答及 1,069 字符最终回答，无 DSML 前导残留；未发起模型 Prompt、未启动 Desktop。此项不等于原生 Kiro 已停止产生该标记。
