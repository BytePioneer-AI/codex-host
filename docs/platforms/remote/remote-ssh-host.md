...
The Renderer resolves each explicit Host ID through Desktop's native Host registry. Only bindings without a registry use a unique matching manager in the committed React tree. A missing, disconnected, or mismatched registry entry never falls back to a stale manager left in another hook. Composer Host identity is separate from connection readiness: losing the remote connection does not make the project local.

Local and remote connections own separate request clients and submission policies. Switching Composers neither disposes the other Host's policy nor copies a local external Harness carrier into remote stock Codex. Requests and switches resolve the native owner immediately, without waiting for the Controller's next poll; periodic reconciliation handles installation and recovery, not Host selection. Connection replacement invalidates only that Host's old client. In-flight requests are not replayed, and retired clients cannot send new requests. After a same-Host reconnection, the visible Composer reapplies its confirmed draft selection; a cross-Host switch reloads the destination Host's Model and Permission Mode catalogs rather than reusing the other Host's catalogs. When a settings overlay temporarily removes all Composers, the last discovered native registry remains queryable, but each lookup reads its current entry rather than reviving a disconnected connection. Conflicting Composer Host identities still prevent an unscoped active-route guess, without blocking explicitly Host-scoped queries.
...
Explicitly unsupported extension methods are remembered only for the current Host request client and method. Those calls fail locally without repeated network probes; unsupported Harness discovery, sidebar ownership, and usage requests do not schedule automatic retries. Account API absence does not disable Harness APIs, and one missing Harness does not disable another. Timeouts, authentication failures, and invalid parameters are not treated as method absence. Replacing that Host's native manager, bridge, or connection policy resets these observations; merely switching the active Composer does not clear observations on the other Host. Supported requests remain concurrent and their results are not cached.
...
`codexhost/harness/inspect is unsupported on this Host connection` means the connection does not provide the codexhost extension method; it is not a failed installation check for Pi or another Harness. Verify that the matching codexhost version is installed and started remotely and that Desktop connects through the managed entrypoint, then reconnect the SSH workspace. Installing only the remote Harness, or seeing `remote status` report ready, does not establish that the existing Desktop connection uses that entrypoint. Stock Codex remains usable after native Thread ownership verification on the same Host.
