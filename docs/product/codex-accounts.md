...
Codex 账号额度耗尽仍然限制原生 Codex，但不应成为独立外部 Harness 的发送额度。Renderer 在单个 Composer 的外部路由、Thread 所有权及配置均已确认、Adapter 就绪且不在切换中时，只忽略该 Composer 的 Codex 账号额度门和 reserve `hardBlocked`，继续走原生提交链路。空输入、附件、运行中、权限等其他原生限制保持不变；外部 Harness 的真实额度与错误仍由其自身处理。

兼容逻辑位于 `renderer-codex-usage-gate.ts`，不依赖当前焦点或固定 hook 序号。它在已提交的 React 树中识别两类布尔订阅，仅投影当前组件的 `getSnapshot`；不会写账号、atom 或额度查询缓存。为取得原生 React 通知回调，会在同步调用栈内临时捕获 `store.sub`，并在 `finally` 中恢复方法；不向共享 store 保留额外订阅。切回 Codex、所有权或配置失效、Composer 移除、扩展卸载时恢复实时原生结果，而非安装时的旧值。无法唯一验证两道门或订阅契约变化时，不安装放行逻辑，也不单独移除发送按钮的 `disabled`；Agent 控件的悬停提示说明保留原生发送限制。切换结束时，临时发送禁用状态从最新原生按钮 props 恢复，避免旧快照覆盖已恢复的 Codex 额度限制。

这是 Desktop 私有订阅契约的版本兼容层，不修改官方账号数据，也不隐藏真实的 Codex 额度横幅。升级审计的 Composer 面记录额度门候选和验证数量；数据尚未加载或版本结构不支持时可能无法验证。模块单元测试覆盖共享 store 的多 Composer 隔离、切换、原生额度更新及恢复；真实跨窗口、附件和后续消息仍需按 Desktop 版本做行为验证。
