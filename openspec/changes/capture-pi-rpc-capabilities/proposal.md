...
- 将原始 Capture 留在忽略目录，仅提交确定性脱敏 Fixture、自动化断言、能力矩阵和 `PASS`、`FAIL` 或 `BLOCKED` Gate 报告。
...
- `pi-rpc-capability-probe`: 定义与 Harness 版本无关的 Pi RPC 启动、传输、生命周期、Session、Tool、Interaction、历史、Model、Fork、脱敏证据和 Gate 判定要求。
...
- 主要影响 `tools/gate-c/`、`tests/fixtures/`、Gate 专用测试、根工程命令和 Pi RPC 验证记录。
