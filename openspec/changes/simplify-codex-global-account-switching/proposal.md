...
- **BREAKING** 设置页提供“当前账号／切换账号”并执行全局切换；Harness 选择器始终只展示一个 Codex Harness，不把保存账号展开成 Harness 选项。Composer 可只读显示当前身份，所有相关窗口同步结果。活跃工作时拒绝切换、不强制中断；切换中的 Codex 请求明确失败或重试，不静默排队后以另一身份执行。
