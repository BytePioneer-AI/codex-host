...
Harness inspection and opened Session capabilities SHALL report `history.rollbackLastTurn=true` only when `HarnessAdapter.open(rollbackLastTurn)` can derive a distinct Native Session whose active history is the source Session's exact current history without its final Turn. The operation SHALL require an idle source with at least one Turn, SHALL allow the derived Snapshot to contain zero Turns, and SHALL NOT modify the source Native Session or project files.
...
- **AND** the source Snapshot and current project files SHALL remain unchanged
...
- **AND** it SHALL remain available for a later Turn
