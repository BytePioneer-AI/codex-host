...
Protocol Core MUST 独占从规范化 `HostUsage` 到当前 Codex app-server `thread/tokenUsage/updated` Notification 的转换。Notification MUST 携带准确的外部 Host Thread ID、关联的活动或最近已完成 Host Turn ID，以及包含 `total`、`last` 和 `modelContextWindow` 的协议有效 `tokenUsage` 对象。只有当规范化快照包含足够可靠的值来构造已评审 carrier 时，Host 才 MUST 发出该 Notification；Host MUST NOT 通过任意 Renderer Request method 暴露 `HostUsage`。
