export const GEMINI_SESSION_UPDATE_EXTENSION_METHODS = [
  "_x.ai/session/update",
  "x.ai/session_notification",
] as const;

export type GeminiCompactionOutcome = "succeeded" | "cancelled" | "failed";

export type GeminiCompactionTransportEvent =
  | {
  ...
  | {
      ...
      outcome: GeminiCompactionOutcome;
      tokensBefore?: number;
      tokensAfter?: number;
      contextWindowTokens?: number;
      errorMessage?: string;
...
export function isGeminiExtensionSessionUpdateMethod(method: string): boolean {
  return (GEMINI_SESSION_UPDATE_EXTENSION_METHODS as readonly string[]).includes(method);
}

export function geminiCompactionEventFromUpdate(
  update: unknown,
): GeminiCompactionTransportEvent | null {
