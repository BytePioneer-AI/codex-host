...
import type { PermissionOption, PromptResponse, RequestPermissionResponse } from "@agentclientprotocol/sdk";
import {
...
import {
  ...
  type OpenCodeAcpTransportOptions,
  type OpenCodeOpenResult,
  type OpenCodePermissionRequest,
  type OpenCodeTransportEvent,
} from "./acp-transport.js";
import { mapOpenCodeMessages, type OpenCodeMessage } from "./opencode-history.js";
import {
  decodeOpenCodeModelId,
  modelStateFromInitialize,
  modelStateFromSessionResponse,
  stateForOpenCodeModel,
...
export interface OpenCodeAdapterDependencies {
  createTransport(options: OpenCodeAcpTransportOptions): OpenCodeAcpTransportLike;
  randomUUID(): string;
}

export interface OpenCodeAcpTransportLike {
  readonly sessionId: string;
  inspect(): Promise<OpenCodeOpenResult["initialize"]>;
  open(input: { kind: "create" } | { kind: "resume"; sessionId: string }): Promise<OpenCodeOpenResult>;
  getHistory(): OpenCodeOpenResult["history"];
  runTurn(
    text: string,
...
function capabilitiesForModels(modelState: OpenCodeModelState): HarnessSessionCapabilities {
  return {
    configuration: {
...
function terminalOutcome(response: PromptResponse, cancelled: boolean): TurnOutcome {
  ...
  return {
    ...
    error: {
      code: "nativeFailure",
      message: `OpenCode stopped the Turn: ${response.stopReason}`,
      retryable:
...
function snapshotFromMessages(
  ...
  return mapOpenCodeMessages(messages, openCodeHarnessId, sessionId, knownTurnRefs);
}
...
class OpenCodeHarnessSession implements HarnessSession {
  ...
  readonly #cwd: string;
  readonly #modelState: OpenCodeModelState;
  readonly #onClosed: () => void;
  readonly #randomUUID: () => string;
  readonly #snapshot: HostThreadSnapshot;
  readonly #toolOutputLimit: number;
  readonly #transport: OpenCodeAcpTransportLike;
  ...
  constructor(
    cwd: string,
    transport: OpenCodeAcpTransportLike,
    opened: OpenCodeOpenResult,
    ...
    this.#state = stateForOpenCodeModel(modelState, { nativeRef: nativeRef(opened.sessionId) });
    this.initialState = this.#state;
    this.#snapshot = {
      ...snapshotFromMessages(
        opened.history?.messages ?? [],
        opened.sessionId,
  ...
  async readSnapshot(): Promise<HarnessResult<HostThreadSnapshot>> {
    ...
    if (this.#active || this.#configuring) {
      return {
        ...
        error: {
          code: "sessionBusy",
          message: "OpenCode Session cannot read history during another operation",
          retryable: true,
        },
      };
    }
  ...
  > {
    ...
    if (command.type === "permissionMode.select") {
      return {
        ...
        error: {
          code: "unsupported",
          message: "OpenCode Permission Mode selection is unavailable",
          retryable: false,
        },
      };
    }
  ...
  handleTransportFault(error: OpenCodeTransportError): void {
    queueMicrotask(() => this.#fault(error));
  }

  async #refreshSnapshot(): Promise<OpenCodeMessage[]> {
    const history = this.#transport.getHistory();
    const knownTurnRefs = this.#snapshot.turns.map((turn) => turn.nativeTurnRef);
    const refreshed = snapshotFromMessages(
      history?.messages ?? [],
  ...
  async #selectModel(command: ModelSelectCommand): Promise<HarnessResult<ModelSelectCompleted>> {
    ...
    if (!available) {
      return {
        ...
        error: {
          code: "invalidRequest",
          message: "OpenCode Model is unavailable",
          retryable: false,
        },
      };
    }
  ...
  async #selectThinking(
    ...
    if (
      ...
      return {
        ...
        error: {
          code: "invalidRequest",
          message: "OpenCode Thinking option is unavailable",
          retryable: false,
        },
      };
    }
  ...
  async #configure(
    ...
    if (this.#active || this.#configuring) {
      return {
        ...
        error: {
          code: "sessionBusy",
          message: "OpenCode Session cannot configure during another operation",
          retryable: true,
        },
      };
    }
    ...
    try {
      await this.#transport.setModel(decodeOpenCodeModelId(model.id), thinkingOptionId);
      this.#state = stateForOpenCodeModel(
        this.#modelState,
  ...
  async #cancel(command: TurnCancelCommand): Promise<HarnessResult<TurnCancelAccepted>> {
    ...
    if (!active || active.command.turnId !== command.turnId) {
      return { ok: false, error: invalidState("OpenCode Turn Cancel must reference the active Turn") };
    }
  ...
  async #respond(
    ...
    if (!active || !pending)
      return { ok: false, error: invalidState("OpenCode Approval is not pending") };
    ...
    if (!option)
      return {
        ...
        error: {
          code: "invalidRequest",
          message: "OpenCode Approval action is unavailable",
          retryable: false,
        },
      };
  ...
  #requestPermission(
    ...
    const interaction: HostApprovalInteraction = {
      ...
      interactionId,
      turnId: active.command.turnId,
      title: request.request.toolCall.title ?? "OpenCode Tool",
  ...
  #handleEvent(active: ActiveTurn, event: OpenCodeTransportEvent): void {
  ...
  #startTool(active: ActiveTurn, event: Extract<OpenCodeTransportEvent, { type: "tool.call" }>): void {
    this.#completeReasoning(active, { status: "succeeded" });
    this.#completeAgent(active, { status: "succeeded" });
  ...
  #updateTool(
    active: ActiveTurn,
    event: Extract<OpenCodeTransportEvent, { type: "tool.update" }>,
  ): void {
    ...
    const outcome: HostItemOutcome =
      status === "failed"
        ? {
            ...
            error: {
              code: "nativeFailure",
              message: `OpenCode Tool '${tool.item.toolName}' failed`,
              retryable: false,
            },
          }
        : { status: "succeeded" };
  ...
  async #settleFromHistory(
    ...
    try {
      ...
      if (created.length !== 1) {
        throw new Error(
          `OpenCode Turn persisted ${created.length} new Native Turns; exactly one is required`,
        );
      }
  ...
  async #close(): Promise<void> {
    ...
    if (this.#active)
      this.#finish(this.#active, {
        status: "failed",
        error: invalidState("OpenCode Session closed during active Turn"),
      });
    this.#phase = "closed";
    this.#channel.end();
  }
...
export class OpenCodeAdapter implements HarnessAdapter {
  ...
  constructor(options: OpenCodeAdapterOptions = {}, dependencies?: OpenCodeAdapterDependencies) {
    ...
    this.#dependencies = dependencies ?? {
      ...
      createTransport: (transportOptions) =>
        new OpenCodeAcpTransport({ ...options, ...transportOptions }),
    };
  }

  async inspect(input: InspectHarnessInput = {}): Promise<HarnessInspection> {
    if (this.#closePromise)
      return { status: "unavailable", error: invalidState("OpenCode Adapter is closed") };
    const cwd = path.resolve(input.cwd ?? process.cwd());
    if (!input.refresh) {
      const cached = this.#inspectionCache.get(cwd);
      if (cached) return cached;
    }
    let transport: OpenCodeAcpTransportLike | null = null;
    try {
      ...
      if (!modelState)
        throw new OpenCodeTransportError("protocolError", "OpenCode returned an invalid Model catalog");
  ...
  async open(input: OpenSessionInput): Promise<HarnessResult<HarnessSession>> {
    if (this.#closePromise) return { ok: false, error: invalidState("OpenCode Adapter is closed") };
    if (input.cwd.length === 0)
      return {
        ok: false,
        error: { code: "invalidRequest", message: "OpenCode Adapter requires cwd", retryable: false },
      };
    if (input.kind === "fork" || input.kind === "rollbackLastTurn") {
      return {
        ...
        error: {
          code: "unsupported",
          message: "OpenCode MVP does not support exact history mutation",
          retryable: false,
        },
      };
    }
    ...
    if (parsedRef && (!parsedRef.success || parsedRef.data.harnessId !== this.harnessId)) {
      return {
        ...
        error: {
          code: "invalidRequest",
          message: "OpenCode cannot resume another Harness's Native Session",
          retryable: false,
        },
      };
    }
    ...
    try {
      ...
      if (input.kind === "create") {
        ...
        if (!catalogModel)
          throw new OpenCodeTransportError("protocolError", "Requested OpenCode Model is unavailable");
        if (
          selectedThinking &&
          ...
          throw new OpenCodeTransportError(
            "protocolError",
            "Requested OpenCode Thinking option is unavailable",
          );
        }
        if (
          ...
          await transport.setModel(decodeOpenCodeModelId(selectedModel.id), selectedThinking);
      ...
      const initialUsage =
        input.kind === "resume" ? lastTurnUsage(opened.history?.messages ?? []) : null;
      const openedSession = new OpenCodeHarnessSession(
        cwd,
        transport,
        opened,
        modelState,
