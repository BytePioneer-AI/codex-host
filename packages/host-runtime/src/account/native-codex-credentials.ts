...
export class NativeCodexCredentials {
  ...
  static parse(serialized: string): NativeCodexCredentials {
    try {
      ...
      const workspaceId = document.tokens.account_id;
      if (
        id.iss !== access.iss ||
        id.sub !== access.sub ||
        id["https://api.openai.com/auth"].chatgpt_account_id !== workspaceId ||
        access["https://api.openai.com/auth"].chatgpt_account_id !== workspaceId
      ) {
        throw new InvalidNativeCodexCredentialsError();
      }
  ...
  managedOAuthCredential(): ManagedCodexOAuthCredential {
    try {
      ...
      return {
        ...
        chatgptAccountId: document.tokens.account_id,
