...
export class OfficialWorkTracker {
  ...
  admitted(client: string, method: string, params: JsonObject): (response: JsonObject) => void {
    const processDescriptor =
      method === "process/spawn" ? key("", "process-handle", params.processHandle) : undefined;
    const sustained =
      method === "process/spawn"
        ? key(client, "process", params.processHandle)
        : method === "thread/realtime/start"
          ? key("", "realtime", params.threadId)
          : method === "thread/compact/start"
            ? key("", "compact", params.threadId)
            : method === "mcpServer/oauth/login"
              ? key("", "oauth", params.name)
              : undefined;
    const alreadyActive = sustained ? this.#gate.nativeWork(sustained, true) : false;
    if (processDescriptor && sustained) {
      const processKeys = this.#processes.get(processDescriptor) ?? new Set<string>();
      processKeys.add(sustained);
      this.#processes.set(processDescriptor, processKeys);
    }
    if (method === "thread/shellCommand") this.#shell(params.threadId, 1);
    const queueKey = key("", "queue", params.threadId);
    const goalKey = key("", "goal", params.threadId);
    ...
    return (response) => {
      ...
      if (response.error) {
        ...
        if (sustained && !alreadyActive) {
          this.#gate.nativeWork(sustained, false);
          if (processDescriptor) this.#forgetProcess(processDescriptor, sustained);
        }
        return;
      }
      ...
      if (startsTurn && (!object(response.result) || !object(response.result.turn)))
        this.#gate.nativeWork(key("", "unconfirmed-start", params.threadId), true);
      ...
      if (
        ...
        this.#gate.nativeWork(key("", "thread-status", result.thread.id), true);
      }
  ...
  notification(client: string, value: JsonObject): void {
    ...
    if ((method === "turn/started" || method === "turn/completed") && object(params.turn)) {
      this.#gate.nativeWork(key("", "unconfirmed-start", params.threadId), false);
      const id = key("", "turn", [params.threadId, params.turn.id]);
    ...
    if (method === "thread/status/changed" && object(params.status)) {
      ...
      this.#gate.nativeWork(
        key("", "thread-status", params.threadId),
        !["idle", "notLoaded"].includes(String(params.status.type)),
      );
    }
    if (method === "thread/realtime/started")
      this.#gate.nativeWork(key("", "realtime", params.threadId), true);
    if (method === "thread/realtime/closed")
      this.#gate.nativeWork(key("", "realtime", params.threadId), false);
    if (method === "process/exited") {
      const descriptor = key("", "process-handle", params.processHandle);
      for (const processKey of this.#processes.get(descriptor) ?? [])
        this.#gate.nativeWork(processKey, false);
      this.#processes.delete(descriptor);
    }
    if (method === "thread/compacted")
      this.#gate.nativeWork(key("", "compact", params.threadId), false);
    if (method === "mcpServer/oauthLogin/completed")
      this.#gate.nativeWork(key("", "oauth", params.name), false);
  ...
  #turn(client: string, thread: unknown, turn: JsonObject): void {
    void client;
    const id = key("", "turn", [thread, turn.id]);
  ...
  #forgetProcess(descriptor: string, processKey: string): void {
    const keys = this.#processes.get(descriptor);
    if (!keys) return;
    keys.delete(processKey);
    if (keys.size === 0) this.#processes.delete(descriptor);
  }
}
