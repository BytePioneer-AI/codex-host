...
export function formatSubagentRowMeta(row: SubagentRowMeta): string | undefined {
  ...
  const output = row.output?.trim().split(/\r?\n/u).at(-1)?.trim();
  const parts = [
    modelLine,
...
function contribute(target: Partial<SubagentRowMeta>, source: Record<string, unknown>): void {
  ...
  if (nonBlank(source.message) && !target.output) target.output = source.message.trim();
  if (nonBlank(source.resultSummary) && !target.output) {
    target.output = source.resultSummary.trim();
  }
  if (isRecord(source.agentsStates) && target.conversationId) {
    const state = source.agentsStates[target.conversationId];
    if (isRecord(state)) {
      if (nonBlank(state.status) && !target.status) target.status = state.status.trim();
      if (nonBlank(state.message) && !target.output) target.output = state.message.trim();
    }
  }
}
...
export function subagentRowMetaFromProps(value: unknown): SubagentRowMeta | null {
  ...
  return {
    ...
    ...(nonBlank(target.output) ? { output: target.output.trim() } : {}),
...
function mergeRow(
  ...
  const output = base.output ?? next.output;
