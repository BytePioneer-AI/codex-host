...
describe("Renderer settings foundation", () => {
  it("publishes deterministic product sections with Connections as the default", () => {
    ...
    expect(pages.map(({ label }) => label)).toEqual(["Connections", "Updates"]);
    expect(pages.map(({ icon }) => icon)).toEqual(["connections", "updates"]);
  ...
  it("publishes only available settings pages", () => {
    ...
    expect(pages.map(({ id }) => id)).toEqual(["connections", "updates"]);
