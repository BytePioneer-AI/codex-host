...
The acceptance command requires a repository-confined report, validates its exact schema, rejects `possible-impact` and `confirmed-impact`, refuses an existing baseline or duplicate identity, and writes JSON by atomic rename. A predeclared identity may populate its explicitly declared missing baseline once. A new identity appends one manifest entry and names its baseline with platform, version, build, and the complete 64-character hexadecimal ASAR digest; existing explicit manifest paths remain compatible. Review the resulting manifest and baseline diff before committing it.
...
The lock protects against normal concurrent acceptance commands and fail-stops after process interruption. It is not a security boundary against an attacker who already controls the same operating-system user and can deliberately replace files between ownership checks.
