...
export function parseReviewedDesktopManifest(value, manifestDirectory) {
  if (
    ...
    !Array.isArray(value.desktops) ||
    value.desktops.length === 0
  ) {
    throw new Error(
      "invalid reviewed desktop manifest: schemaVersion 1 and non-empty desktops required",
    );
  }
  for (let index = 0; index < value.desktops.length; index += 1) {
    if (!Object.hasOwn(value.desktops, index))
      throw new Error("desktop entries must not be sparse");
  }
  ...
  const desktops = value.desktops.map((entry) => {
    ...
    if (!SHA256.test(entry.asarIntegrity))
      throw new Error("asarIntegrity must be lowercase sha256");
  ...
  return { schemaVersion: 1, manifestDirectory: root, desktops };
}

export function findReviewedDesktop(manifest, identity) {
  const match = manifest?.desktops?.find((entry) =>
